...
def create_metadata(image_url, myname):
    metadata = {"name": name, "description": description, "image": image_url}
    metadata_filepath = file_root + myname.split(" ")[0] + ".json"
...
def issuecertificates():
    ...
    metadata_filepath = create_metadata(image_url, myname)
    metadata_url = upload_file(metadata_filepath)
    issuer_account = "0xAf47726af31C42ef57c771ea078D41cF0B0024A2"
    this_issuer = issuer.objects.filter(account=issuer_account).first()
