...
def template_base_image(instance, filename):
    return "/".join(
        [str(instance.user.account), "template_base_image", instance.name, ".png"]
    )


def certificate_image(instance, filename):
    return "/".join(
        [str(instance.user.account), "certificates", str(instance.id) + ".png"]
    )


def certificate_json(instance, filename):
    return "/".join(
        [str(instance.user.account), "certificates", str(instance.id) + ".json"]
    )
...
class Approver(models.Model):
    ...
    idProofApprover = models.ImageField(
        upload_to=idProofApprover, blank=True, null=True
    )
...
class Batch(models.Model):
    ...
    batch_nft_image = models.ImageField(
        upload_to=batch_nft_image, blank=True, null=True
    )
    batch_nft_metadata = models.FileField(
        upload_to=batch_nft_image, blank=True, null=True
    )
